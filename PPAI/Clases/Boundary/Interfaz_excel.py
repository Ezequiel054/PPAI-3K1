...
class Interfaz_excel:
    ...
    def exportar_excel(self, datos):
        ...
        max_varietals = max(len(row[4]) for row in datos)  # Calcula la longitud máxima de las variedades
        for i in range(1, max_sommelier_scores + 1):
            headers_sommelier.append(f'Puntaje Sommelier {i}')

        headers_variedades = []
        for i in range(1, max_varietals + 1):  # Utiliza la longitud máxima de las variedades
        ...
        for row in datos[:10]:
            ...
            varietals = row[4] + [None] * (max_varietals - len(row[4]))  # Ajusta el número de variedades
